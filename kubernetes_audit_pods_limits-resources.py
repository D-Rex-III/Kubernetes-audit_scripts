...
for namespace in json_object["items"]:
    ...
    json_object = execute_kube_command_json(command)
    audit_report_request_limits[namespace_name] = {}
    if not json_object["items"]:
        ...
        for pod in json_object["items"]:
            audit_report_request_limits[namespace_name][pod["metadata"]["name"]] = {}
            for container in pod["spec"]["containers"]:
                audit_report_request_limits[namespace_name][pod["metadata"]["name"]][container["name"]] = {}
                if "request" in container and "limits" in container:
                    audit_report_request_limits[namespace_name][pod["metadata"]["name"]][container["name"]] = {
                        "request": "present",
                        "limits": "present"
                    }
                elif "request" in container and "limit" not in container:
                    audit_report_request_limits[namespace_name][pod["metadata"]["name"]][container["name"]] = {
                        "request": "not present",
                        "limits": "present"
                    }
                elif "request" not in container and "limits" in container:
                    audit_report_request_limits[namespace_name][pod["metadata"]["name"]][container["name"]] = {
                        "request": "present",
                        "limits": "not present"
                    }
...
print(audit_report_request_limits)